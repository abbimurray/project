...
public class ReservationDao {
    ...
    public List<Reservation> getReservationsByCustomerId(int customerID) {
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            while (rs.next()) {
                ...
                reservation.setChargerID(rs.getInt("chargerID"));
                reservation.setCustomerID(rs.getInt("customerID"));
                reservation.setReservationStartTime(rs.getTimestamp("reservationStartTime").toLocalDateTime());
                reservation.setReservationEndTime(rs.getTimestamp("reservationEndTime").toLocalDateTime());
                reservation.setStatus(rs.getString("status"));
                reservations.add(reservation);
            }
        } catch (SQLException e) {
    ...
    public boolean deleteReservation(int reservationID) {
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            return affectedRows > 0; // true if it was deleted successfully
        } catch (SQLException e) {
            e.printStackTrace();
            return false;
        }
    ...
    public boolean isChargerAvailable(int chargerID, LocalDateTime startTime, LocalDateTime endTime) {
        ...
        try (Connection conn = DBConnection.getConnection(); PreparedStatement pstmt = conn.prepareStatement(sql)) {
            ...
            ResultSet rs = pstmt.executeQuery();
            if (rs.next()) {
                return rs.getInt(1) == 0;  // True if there are no overlapping reservations
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
